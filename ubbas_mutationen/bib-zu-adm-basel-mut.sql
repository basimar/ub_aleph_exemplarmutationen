...
/******************************************************************************/
/******************************************************************************/
/*  ACHTUNG: Dieses SQL-Script wird woechentlich fuer Basler                  */
/*  Exemplaraenderungen verwendet (ausgefuehrt von:                           */
/*  /dsv51/scripts/ubbas_mutationen/ubbas_mutationen.sh)                      */
...
SPOOL $alephe_scratch/bibkey_reind_basel_mut.sys

select distinct lpad(z103_lkr_doc_number,9,'0')||'DSV01' from z103, itemkeys_basel_mut
  where z103_lkr_library='DSV01'
  and Z103_LKR_TYPE = 'ADM'
  and z103_rec_key_1 = 'DSV51'||substr(itemkey,1,9)                  
  and substr(itemkey,1,9) in (select substr(itemkey,1,9) from itemkeys_basel_mut)
